...
USER_AGENT = "HVACLeadResearch/0.2 (respectful local crawler)"
...
def extract_emails(page_html: str) -> set[str]:
    ...
    rejected_domains = {"example.com", "example.org", "sentry.io"}
    rejected_domain_fragments = ("sentry", "wixpress.com")
...
def crawl_company(company: dict[str, str]) -> dict[str, set[str]]:
    """Crawl homepage and up to three contact/about pages."""
    ...
    with build_session() as session:
        ...
        for page_url in discover_contact_pages(home_url, home_html):
            try:
